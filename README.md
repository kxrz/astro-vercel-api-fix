...
**Delete or remove**:
- ❌ `vercel.json` (unless absolutely necessary for non-Astro features)
- ❌ Native Vercel functions in `api/*.ts` directory
- ❌ Custom build scripts that modify `dist/server/`
- ❌ Manual rewrites or function declarations
...
```
src/pages/api/
├── contact.ts       → export const prerender = false;
├── newsletter.ts    → export const prerender = false;
└── download-pdf.ts  → export const prerender = false;
```
...
5. **Log strategically**: Use prefixed logs (`📧 [Route]`) to track execution
